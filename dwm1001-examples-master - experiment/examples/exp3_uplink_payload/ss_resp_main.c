...
*           This code implements SS TWR responder with two uplink payload modes switching and data recording
*
* @attention
*
...
#define APP_NAME "SS TWR RESP Exp3 v1.0"
...
/* Current uplink mode */
static volatile uplink_mode_t current_mode = UPLINK_MODE_A;
static volatile uplink_mode_t pending_mode = UPLINK_MODE_A;
...
static uint32_t last_bandwidth_calc_time_ms = 0;
static uint32_t last_bandwidth_bytes = 0;

/* Transaction Counters */
static volatile uint32_t rx_count = 0;
static volatile uint32_t tx_count = 0;
static volatile uint32_t uplink_count = 0;
...
static void reset_mode_statistics(uplink_mode_t mode);
static void update_statistics(uplink_mode_t mode, int32_t rssi, uint16_t rxpacc, int16_t snr, uint32_t uplink_bytes);
static void simulate_uplink_transmission(uplink_mode_t mode, uint64 timestamp);
...
/*! ------------------------------------------------------------------------------------------------------------------
* @fn get_uplink_payload_size()
*
* @brief Get uplink payload size for a given mode
*/
uint32_t get_uplink_payload_size(uplink_mode_t mode)
{
    if (mode == UPLINK_MODE_A) {
        return UPLINK_A_PAYLOAD_SIZE;
    } else {
        return UPLINK_B_PAYLOAD_SIZE;
    }
}

/*! ------------------------------------------------------------------------------------------------------------------
* @fn simulate_uplink_transmission()
*
* @brief Simulate uplink transmission to server (in real system, this would be via WiFi/LoRa/etc.)
*        For this experiment, we just record the data size and count
*/
static void simulate_uplink_transmission(uplink_mode_t mode, uint64 timestamp)
{
    uint32_t payload_size = get_uplink_payload_size(mode);
    
    /* In a real system, this would send data via WiFi/LoRa/other uplink interface */
    /* For this experiment, we simulate by just recording the bytes */
    
    uplink_count++;
    uplink_bytes_total += payload_size;
    
    /* Calculate bandwidth periodically */
    uint32_t current_time = get_system_time_ms();
    uint32_t elapsed = current_time - last_bandwidth_calc_time_ms;
    
    if (elapsed >= 1000) {  /* Calculate bandwidth every second */
        uint32_t bytes_in_period = uplink_bytes_total - last_bandwidth_bytes;
        uint32_t bytes_per_sec = (bytes_in_period * 1000) / elapsed;
        
        /* Update statistics */
        if (mode < 2) {
            mode_stats[mode].uplink_bytes_per_sec = bytes_per_sec;
        }
        
        last_bandwidth_calc_time_ms = current_time;
        last_bandwidth_bytes = uplink_bytes_total;
    }
}
...
*
* @brief Check if it's time to switch uplink mode
*/
static void check_mode_switch(void)
{
    uint32_t current_time = get_system_time_ms();
    ...
    {
        /* Time to switch mode */
        uplink_mode_t next_mode = (current_mode == UPLINK_MODE_A) ? UPLINK_MODE_B : UPLINK_MODE_A;
        ...
        printf("Uplink mode switch requested: %d (payload size: %d bytes)\r\n", 
               next_mode, get_uplink_payload_size(next_mode));
        
        /* Wait a short time for main.c monitor task to detect */
        ...
        last_bandwidth_calc_time_ms = get_system_time_ms();
        last_bandwidth_bytes = uplink_bytes_total;
        ...
        printf("Uplink mode switched to: %d\r\n", current_mode);
...
static void update_statistics(uplink_mode_t mode, int32_t rssi, uint16_t rxpacc, int16_t snr, uint32_t uplink_bytes)
{
    ...
    mode_stats[mode].uplink_count = uplink_count;
    mode_stats[mode].uplink_bytes = uplink_bytes_total;
...
/*! ------------------------------------------------------------------------------------------------------------------
* @fn set_current_uplink_mode()
*
* @brief Set current uplink mode
*/
void set_current_uplink_mode(uplink_mode_t mode)
{
    ...
    printf("Uplink mode set to: %d (payload size: %d bytes) at time %lu ms\r\n", 
           mode, get_uplink_payload_size(mode), mode_start_time_ms);
}

/*! ------------------------------------------------------------------------------------------------------------------
* @fn get_current_uplink_mode()
*
* @brief Get current uplink mode
*/
...
/*! ------------------------------------------------------------------------------------------------------------------
* @fn get_pending_uplink_mode_switch()
*
* @brief Get pending mode switch (if any)
*/
uplink_mode_t get_pending_uplink_mode_switch(void)
{
    if (mode_switch_pending) {
        return pending_mode;
    }
...
/*! ------------------------------------------------------------------------------------------------------------------
* @fn clear_uplink_mode_switch_pending()
*
* @brief Clear the pending mode switch flag
*/
void clear_uplink_mode_switch_pending(void)
{
    mode_switch_pending = 0;
}
...
{
    uplink_mode_t mode = current_mode;
    const char *mode_name = (mode == UPLINK_MODE_A) ? "A (TS only)" : "B (TS+CIR_s)";
    uint32_t payload_size = get_uplink_payload_size(mode);
    
    printf("\r\n========================================\r\n");
    printf("Statistics for Uplink mode %s:\r\n", mode_name);
    printf("Payload size: %d bytes\r\n", payload_size);
    printf("========================================\r\n");
    printf("RX Count: %lu\r\n", mode_stats[mode].rx_count);
    printf("TX Count: %lu\r\n", mode_stats[mode].tx_count);
    printf("Uplink Count: %lu\r\n", mode_stats[mode].uplink_count);
    printf("Total Uplink Bytes: %lu\r\n", mode_stats[mode].uplink_bytes);
    printf("Uplink Bandwidth: %lu bytes/sec\r\n", mode_stats[mode].uplink_bytes_per_sec);
    printf("Uplink Bandwidth: %.2f kbps\r\n", mode_stats[mode].uplink_bytes_per_sec * 8.0 / 1000.0);
    
    if (mode_stats[mode].uplink_count > 0) {
        printf("Avg Uplink Bytes per TX: %.2f bytes\r\n", 
               (float)mode_stats[mode].uplink_bytes / mode_stats[mode].uplink_count);
    }
...
{
    const char *mode_name = (record->mode == UPLINK_MODE_A) ? "A" : "B";
    
    printf("[%lu ms] UPLINK_%s: RX=%lu TX=%lu Uplink=%lu Bytes=%lu BW=%lu bps (%.2f kbps)\r\n",
           record->timestamp_ms,
           mode_name,
           record->rx_count,
           record->tx_count,
           record->uplink_count,
           record->uplink_bytes,
           record->uplink_bytes_per_sec,
           record->uplink_bytes_per_sec * 8.0 / 1000.0);
}
...
int ss_resp_run(void)
{
    ...
    if (current_time - last_record_time_ms >= RECORD_INTERVAL_MS) {
        ...
        record.uplink_count = uplink_count;
        record.uplink_bytes = uplink_bytes_total;
        record.uplink_bytes_per_sec = mode_stats[current_mode].uplink_bytes_per_sec;
        record.valid = 0;
    ...
    if (status_reg & SYS_STATUS_RXFCG)
    {
        ...
        if (memcmp(rx_buffer, rx_poll_msg, ALL_MSG_COMMON_LEN) == 0)
        {
            ...
            if (ret == DWT_SUCCESS)
            {
                ...
                /* Simulate uplink transmission to server */
                simulate_uplink_transmission(current_mode, poll_rx_ts);
                
                /* Update statistics */
                update_statistics(current_mode, rssi, rxpacc, snr, get_uplink_payload_size(current_mode));
                ...
                if (tx_count % 10 == 0) {
                    printf("[%lu] TX#%lu: Uplink#%lu Payload=%d bytes RSSI=%ld RXPACC=%d SNR=%d\r\n",
                           current_time, tx_count, uplink_count, 
                           get_uplink_payload_size(current_mode), rssi, rxpacc, snr);
                }
            }
            else
            {
...
void ss_responder_task_function (void * pvParameter)
{
    ...
    printf("Experiment will run for %d seconds per uplink mode\r\n", EXPERIMENT_DURATION_MS / 1000);
    printf("Mode switch delay: %d seconds\r\n", MODE_SWITCH_DELAY_MS / 1000);
    printf("Uplink mode A: TS only (%d bytes)\r\n", UPLINK_A_PAYLOAD_SIZE);
    printf("Uplink mode B: TS+CIR_s (%d bytes)\r\n", UPLINK_B_PAYLOAD_SIZE);
...
*    - Records: uplink_bytes, uplink_bytes_per_sec (bandwidth), uplink_count
*    - Each mode runs for 150 seconds
*    - 10 second delay between mode switches
*    - Statistics printed at mode switch
*    - A组：仅时间戳（8字节），更省电、带宽更低
*    - B组：时间戳+CIR摘要（104字节），功耗略高但更易做定位质量诊断
*
****************************************************************************************************************************************************/
