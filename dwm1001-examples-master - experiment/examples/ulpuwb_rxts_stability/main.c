...
static dwt_config_t config = {
    ...
    (4097 + 64 - 64)     /* SFD timeout (preamble length + 1 + SFD length - PAC size). Used in RX only. */
};

static uint8 tx_poll_msg[15] = {0x44, 0x00, 0x05, 0x00, 0x00, 0x00, 'A', 'V', 'E', 0xE0, 0, 0};

/* Preamble timeout, in multiple of PAC size. See NOTE 3 below. */
#define PRE_TIMEOUT 1000

/* Delay between frames, in UWB microseconds. See NOTE 1 below. */
#define POLL_TX_TO_RESP_RX_DLY_UUS 100000 
...
int main(void)
{
  ...
  /*Initialization UART*/
  boUART_Init ();
  printf("HopWINS ULPUWB RX TimeStamp Stability Test\r\n");
  ...
  * As this example only handles one incoming frame with always the same delay and timeout, those values can be set here once for all. */
  ...
  dwt_setrxtimeout(00); // Maximum value timeout with DW1000 is infinite  
  ...
  // IF WE GET HERE THEN THE LEDS WILL BLINK
  ...
  #ifdef  NodeRoleTX
    uint8 tx_seq = 0;
    uint32 delayedtxtime = dwt_readsystimestamphi32();
    while (1)
    {
      dwt_write32bitreg(SYS_STATUS_ID, SYS_STATUS_TXFRS);
      tx_poll_msg[1] = tx_seq;
      dwt_writetxdata(sizeof(tx_poll_msg), tx_poll_msg, 0); /* Zero offset in TX buffer. */
      dwt_writetxfctrl(sizeof(tx_poll_msg), 0, 1); /* Zero offset in TX buffer, ranging. */

      /* Start transmission, indicating that a response is expected so that reception is enabled automatically after the frame is sent and the delay
      * set by dwt_setrxaftertxdelay() has elapsed. */
      delayedtxtime += 0x01000000;    // 0x01000000 / 249.6MHz = 67.2ms
      dwt_setdelayedtrxtime(delayedtxtime);
      dwt_starttx(DWT_START_TX_DELAYED);
      printf("TXTS=%08lX\r\n", delayedtxtime);
      while (!(dwt_read8bitoffsetreg(SYS_STATUS_ID, SYS_STATUS_OFFSET) & SYS_STATUS_TXFRS));
      tx_seq += 1;
    }
  #endif
    ...
    while (1)
    {
      ...
      {		
        ...
        uint64 rxtime = get_rx_timestamp_u64();
        printf("RXTS=%02lX%08lX\r\n", (uint32)(rxtime>>32), (uint32)(rxtime));
      }
      else
      {
